...
{
	if (this->grade <= sample.getGrade())
	{
		std::cout << "Bureaucrat, named: '" + this->name + "' signs '" + sample.getName() + "'" << std::endl;
		sample.setSign();
	}
	else
		std::cout << "Bureaucrat, named: '" + this->name + "' failed signation of this '" + sample.getName() + "' - grade is too low" << std::endl;
}
